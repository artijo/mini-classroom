...
<body>
	<header>
		<div class="topNav bg-[#474F7A] h-24 mb-10 pl-10 flex items-center gap-5">
			...
			<h1 class="text-xl font-semibold text-white">แก้ไขงานในชั้นเรียน</h1>
		</div>
	</header>
	...
	<div class="container max-w-5xl mx-auto mb-20">
		<div>
			<form action="/roomTeacher/editAss/${listAss[0].idAssignment}" method="post" enctype="multipart/form-data" class="flex gap-5">
			    <input name="roomId" value="${listAss[0].room.idRoom}" hidden="hidden" >
			    <div class="leftside  border rounded-lg mx-auto p-8 w-3/4 ">
			        <div class="mb-5">
			            ...
			            focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" value="${listAss[0].title}">
			        </div>
			        
			        <div class="mb-5">
			            <label class="block mb-2 text-sm font-medium text-gray-900">รายละเอียด</label>
			            <textarea name="detail" rows="4" class="block resize-none p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 h-56">${listAss[0].detail}</textarea>
			        </div>
			        <div class="mb-5">
			    ...
			    <div class="rightside border rounded-lg mx-auto p-8 h-fit">
			        <div class="mb-5">
			            ...
			            focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"  value="${listAss[0].dueDate}">
			        </div>
			        <div class="mb-5">
			            <label class="block mb-2 text-sm font-medium text-gray-900">คะแนนเต็ม</label>
			            <input type="number" value="${listAss[0].fullScore}" name="fullScore" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg 
